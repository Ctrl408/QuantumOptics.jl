...
Subtypes of this abstract type correspond to concrete routines.
See `LapackDiag`, `KrylovDiag` for more info.
"""
...
Represents the `KrylovKit.eigsolve` routine.
Implements the Lanczos & Arnoldi algorithms.
"""
...
- `v0`: The starting vector. By default it is `nothing`, which means it will be a random dense `Vector`.
  This will not work for non-trivial array types like from `CUDA.jl`, so you might want to define a new
  method for the `QuantumOptics.get_starting_vector` function.
...
By default `n` is equal to the matrix size for dense matrices; for sparse
matrices the default value is 6.

This is just a thin wrapper around julia's `LinearRgebra.eigen` and
`KrylovKit.eigsolve` functions. Which of them is used depends on the type
of the given operator. If more control about the way the calculation is done
is needed, use the method instance with `DiagStrategy` (see below).
...
If the given operator is non-hermitian a warning is given. This behavior can
be turned off using the keyword `warning=false`.
...
eigenstates(op::AbstractOperator, n::Int; warning=true, kw...) = eigenstates(op; warning=warning, kw..., n=n)
...
operator using the `ds` diagonalization strategy. The `kw...` arguments can be
passed to the exact function that does the diagonalization (like `KrylovKit.eigsolve`).
"""
function eigenstates(op::Operator, ds::LapackDiag; warning=true)
    b = basis(op)
    # Check hermiticity more carefully for dual numbers
    is_herm = try
        ishermitian(op)
    catch
        false
    ...
    if is_herm
        # For dual numbers, we may need to avoid the Hermitian wrapper
        try
            D, V = eigen(Hermitian(op.data), 1:ds.n)
            states = [Ket(b, V[:, k]) for k=1:length(D)]
            return D, states
        catch
            # Fallback for types that don't support Hermitian wrapper
            D, V = eigen(op.data)
            states = [Ket(b, V[:, k]) for k=1:length(D)]
...
function eigenstates(op::Operator, ds::KrylovDiag; warning::Bool=true, kwargs...)
    ...
    is_herm || (warning && @warn(nonhermitian_warning))
